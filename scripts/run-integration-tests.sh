...
echo "TIMELINE: Default CNI integration tests took $DURATION seconds."
...
echo "TIMELINE: Updating CNI image took $DURATION seconds."
...
echo "TIMELINE: Current image integration tests took $DURATION seconds."
...
if [[ $TEST_PASS -eq 0 && "$RUN_CONFORMANCE" == true ]]; then
  ...
  echo "TIMELINE: Conformance tests took $DURATION seconds."
fi
...
if [[ "$DEPROVISION" == true ]]; then
    ...
    echo "TIMELINE: Down processes took $DURATION seconds."
fi
